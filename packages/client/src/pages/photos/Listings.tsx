...
export default function PhotoListings() {
    ...
    return (
        <main className="my-6 flex flex-col justify-center bg-slate-50 overflow-hidden max-w-8xl w-full px-24 space-y-6">
            <div className="grid sm:grid-cols-2 md:grid-cols-3">
                {
                    photos?.map((photo, index) => (
                        ...
                        >
                            <img key={photo?._id} className="w-full rounded object-cover cursor-pointer" src={photo?.url} width="232" height="290" alt={`Image ${photo?.filename}`} />
